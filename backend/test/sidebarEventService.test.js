...
function createTempServices(testName) {
  ...
  const databasePath = path.resolve(tempDir, `${testName}.db`);
...
test("suppresses duplicate sidebar snapshots and keeps changed unread count", () => {
  ...
  try {
    const basePayload = {
      ...
      capturedAt: "2026-07-11T20:00:01.000Z",
      rawMeta: {
        source: "sidebar-test"
      }
    };
    ...
    const duplicate = context.sidebarEventService.save({
      ...basePayload,
      capturedAt: "2026-07-11T20:05:00.000Z"
    });
    const changedUnread = context.sidebarEventService.save({
      ...basePayload,
      unreadCount: 2,
      capturedAt: "2026-07-11T20:06:00.000Z"
    });
